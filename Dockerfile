# Dockerfile para Menu Digital - Otimizado para Next.js 15
...
RUN \
  if [ -f package-lock.json ]; then npm ci --omit=dev --frozen-lockfile; \
...
# Configurar para produção standalone
...
# Fazer uso automático dos outputs trace para reduzir tamanho da imagem
...
# server.js é criado pelo output standalone
CMD ["node", "server.js"]
